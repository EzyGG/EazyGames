...
class User:
    ...
    def get_points(self) -> int:
        connect.execute(f"""SELECT points FROM users WHERE uuid="{self.uuid}\"""")
        return connect.fetch(1)[0]

    def set_points(self, points: int):
        connect.execute(f"""UPDATE users SET points = {points} WHERE uuid="{self.uuid}\"""")
        connect.commit()

    def add_points(self, points: int):
        connect.execute(f"""UPDATE users SET points = {self.get_points() + points} WHERE uuid="{self.uuid}\"""")
        connect.commit()
